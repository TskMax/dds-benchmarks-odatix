...
# Execution command: python3 publi_Tierney_QuarterWave_R4.py

# ==========================================
# 1. SCRIPT PARAMETERS
# ==========================================
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
DEFAULT_ARCHITECTURE = 'Publi_Tierney_QuarterWave_R4'  # Folder name (e.g., FullWave, QuarterWave)

# FORCE YOUR OWN DISPLAY NAME HERE FOR THE PUBLICATION TITLE
DISPLAY_NAME = "Tierney Quarter-Wave R4"  # Customize this string as you wish!
...
def load_data(file_path):
    ...
    # Liste des configurations autorisées
    allowed_configs = {'3335', '3344', '4433', '5333'}
    ...
    for techno, archs in fmax_results.items():
        for arch, bits_dict in archs.items():
            for bit_str, metrics in bits_dict.items():
                ...
                # Filtrage : on ne garde que les configs numériques types 3335
                if bit_str not in allowed_configs:
                    continue
                
                row = {
                    'Technology': techno,
                    'Architecture': arch,
                    'Resolution_Bits': bit_str, # On garde le nom tel quel
    ...
    # Tri manuel pour garantir l'ordre : 3335, 3344, 4433, 5333
    df['Resolution_Bits'] = pd.Categorical(df['Resolution_Bits'], categories=sorted(list(allowed_configs)), ordered=True)
...
def plot_combined_figure(df, area_unit, fmax_unit, power_unit, output_dir, arch_name, display_name):
    ...
    color2 = 'tab:red'
    # Axe X mis à jour pour être plus générique
    ...
    ax3.set_xticks(df_0v9['Resolution_Bits'].unique())
    # Rotation des étiquettes si elles sont trop longues (ex: '3335')

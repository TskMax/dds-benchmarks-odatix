...
# Commande de compilation : python3 plot_odatix_excessfour.py
...
DEFAULT_ARCHITECTURE = 'ExcessFour'  # Adapte au nouveau fichier YAML       
...
def charger_donnees(chemin_fichier):
    ...
    for techno, archs in resultats_fmax.items():
        for arch, bits_dict in archs.items():
            for bit_str, metrics in bits_dict.items():
                ...
                # Nettoyage insensible a la casse : "16bits" ou "16Bits" -> 16
                bit_val = int(bit_str.lower().replace('bits', ''))
    ...
    # Conversion de l'aire de um2 a mm2
    if 'Cell_Area' in df.columns:
        df['Cell_Area'] = df['Cell_Area'] / 1_000_000.0
    
    # Remplacement de l'unite pour l'affichage graphique
...
def tracer_courbe(df, metrique_y, label_y, titre, output_dir, nom_arch):
    ...
    labels_propres = [l.split('_')[-1] for l in labels] # Ex: garde juste "0V8" ou "0V9"
    plt.legend(handles, labels_propres, title="Tension (Vdd)")
    
    plt.title(titre, pad=15, fontweight='bold')
    plt.xlabel('Largeur de l\'accumulateur (Bits)')
    plt.ylabel(label_y)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='Nom du dossier d\'architecture (ex: Tchebychev, QuarterWave).'
    )
...
if __name__ == "__main__":
    ...
    unite_area = unites.get('Cell_Area', 'um2')
    tracer_courbe(
        df=df_resultats, 

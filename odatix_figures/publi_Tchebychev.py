...
# Execution command: python3 publi_Tchebychev.py

# ==========================================
# 1. SCRIPT PARAMETERS
# ==========================================
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
# Default folder parameter for the Tchebychev architecture files
DEFAULT_ARCHITECTURE = 'Publi_Tchebychev'  

# FORCE YOUR OWN DISPLAY NAME HERE FOR THE PUBLICATION TITLE
DISPLAY_NAME = "Chebyshev Polynomial Approximation"  
...
def load_data(file_path):
    ...
    # Target specifically the Tchebychev block
    target_arch = 'Tchebychev'
    
    for techno, archs in fmax_results.items():
        if target_arch not in archs:
            continue
        
        bits_dict = archs[target_arch]
        for bit_str, metrics in bits_dict.items():
            if not metrics: 
                continue 
            
            # Clean the 'bits' suffix to get the pure integer value for plotting
    ...
    # Ensure correct sequential sorting along the X-axis

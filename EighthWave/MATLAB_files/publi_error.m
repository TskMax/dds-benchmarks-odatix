...
accumWidth = 14;       % 18 bits
phaseOutWidth = 14;    % 15 bits
ampWidth   = 12;       
fcw        = 164;      % Exact simulation value
f_ech      = 100e6;    
max_amp = (2^(ampWidth - 1)) - 1; 

%% 2. Load Hardware Data (Chisel)
try
    y_chisel = load('dds_output_odatix.txt'); 
catch
    error('File dds_output_odatix.txt not found or badly formatted.');
end
y_chisel = y_chisel(:); 
...
drawnow;
exportgraphics(gcf, 'spectral_analysis_chisel_EightWave.pdf', 'ContentType', 'vector');
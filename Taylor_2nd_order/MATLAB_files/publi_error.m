...
accumWidth = 21;       % 18 bits
phaseOutWidth = 18;    % 15 bits
ampWidth   = 16;       
fcw        = 20971;      % Exact simulation value
f_ech      = 100e6;    
max_amp = (2^(ampWidth - 1)) - 1; 

%% 2. Load Hardware Data (Chisel)
try
    y_chisel = load('sin_dds_output_18bits.txt'); 
catch
    error('File sin_dds_output_base.txt not found or badly formatted.');
end
idx_neg = y_chisel > max_amp;
...
title(sprintf('Chisel Output Spectrum\nSFDR = %.2f dBc | SNR = %.2f dB', sfdr_chisel, snr_chisel))% Augmenter la marge en bas pour préserver l'axe X


ax = gca;
ax.Position = [0.12, 0.18, 0.82, 0.70]; % [gauche, bas, largeur, hauteur]

% Force MATLAB to update the bounding box BEFORE export
drawnow;
exportgraphics(gcf, 'spectral_analysis_chisel_Taylor2_18bits.pdf', 'ContentType', 'vector');
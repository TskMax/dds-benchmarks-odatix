...
class CompressionEightR4PAC(val config: DdsConfig) extends Module {
  ...
  val addrWidth = phaseWidth - 3  // On ampute 3 bits pour l'adressage 8th wave (ex: 16 - 3 = 13 bits)
  
  val rawAddress = io.phaseIn(addrWidth - 1, 0)
  
  // Inversion de l'adresse si on est dans un octant impair (lecture a l'envers pour la symetrie)
  val actualAddressReg = RegNext(
    Mux(ewBit, ((1 << addrWidth) - 1).U(addrWidth.W) - rawAddress, rawAddress)
  )
  ...
  // On calcule les offsets pour couper correctement le bus d'adresse. Par exemple sh4 est le LSB, on prend les derniers bits, pas besoin d'offset
  val sh4 = 0              // LSB     
  val sh3 = W4
  val sh2 = W4 + W3
  ...
  val guardBits = 0                                         // On rajoute 3 bits fractionnaires supplementaires pour garder de la precision dans les calculs internes. 
  val romAmpWidth = ampWidth + guardBits                    // 12 + 3 = 15 bits > 12 bits donc on aura plus de precision. On tronquera le resultat final pour avoir les 12 bits. 
  val romMaxAmp = ((1 << (romAmpWidth - 1)) - 1).toDouble 

  def makeTable(depthBits: Int, shift: Int, addHalfLsb: Boolean) = {  // depthBits correspond ici a W1, W2, etc.. shift a sh1, sh2, etc...
    ...
    val cosT = VecInit(Seq.tabulate(depth) { i =>
      ...
      // EIGHTH WAVE : L'echelle mathematique est ecrasee a Math.PI / 4.0 (0 a 45 degres)
      val rad = (scaledVal / tableMax) * (Math.PI / 4.0)                 // [(i+0,5)*2**sh]/2**addrWidth * (pi/4)
    ...
    val sinT = VecInit(Seq.tabulate(depth) { i =>
      ...
      // EIGHTH WAVE : L'echelle mathematique est ecrasee a Math.PI / 4.0
      val rad = (scaledVal / tableMax) * (Math.PI / 4.0)
      Math.round(Math.sin(rad) * romMaxAmp).toInt.S(romAmpWidth.W)
    })
    (cosT, sinT)
  }

  // Generations des ROMs
  ...
  // Decoupage du bus d'adresse selon les tailles des partitions
  val addr1 = actualAddressReg(addrWidth - 1, sh1)   
  val addr2 = actualAddressReg(sh1 - 1, sh2)         
  val addr3 = actualAddressReg(sh2 - 1, sh3)         
  val addr4 = actualAddressReg(sh3 - 1, 0)           

  // Etage de lecture synchrone des tables ROM
  ...
  val shift1 = romAmpWidth - 1   // car c1*c2 est de taille 15+15 Bits, on elimine donc les 14 fractionnaires de poids faibles (1 bit de signe et 14 bits fractionnaires)
  ...
  val m_s3c4 = RegNext(((s3 * c4) + round1) >> shift1) // sin(theta_3) * cos(theta_2)
  ...
  val interWidth = romAmpWidth + 1  // +1 Bits pour les additions afin d'eviter les overflows
  ...
  val shift2 = 2 * (romAmpWidth - 1) - (ampWidth - 1)       // decalage final pour normaliser : c12*c34 = 2 * (romAmpWidth - 1) bits et ampWidth - 1 bits represente la taille de la sortie
  ...
  // Si isCosEq = 1, l'angle cible se situe dans un octant complementaire (ex: pi/4 a pi/2)
  // On applique les identites de co-fonctions trigonomatriques :
  // preSignCos = cos(pi/2 - x) = sin(x) -> Recoit sinRaw
  // preSignSin = sin(pi/2 - x) = cos(x) -> Recoit cosRaw
  val preSignCos = Mux(isCosEqReg5, sinRaw, cosRaw)
  val preSignSin = Mux(isCosEqReg5, cosRaw, sinRaw)

  // Sortie finale avec application des quadrants de signe
  io.cosOut := RegNext(Mux(cosSignReg5, -preSignCos, preSignCos))
  io.sinOut := RegNext(Mux(sinSignReg5, -preSignSin, preSignSin))
}

object GenerateCompressionEightR4PAC extends App {
  val myConfig = DdsConfigs.activeConfig

  _root_.circt.stage.ChiselStage.emitSystemVerilogFile(
    new CompressionEightR4PAC(myConfig),
